/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */
package com.Digis01.ProgramacionNCapasJulio24.DAO;

import com.Digis01.ProgramacionNCapasJulio24.ML.Estado;
import com.Digis01.ProgramacionNCapasJulio24.ML.Result;
import java.sql.ResultSet;
import java.sql.Types;
import java.util.ArrayList;
import java.util.List;
import javax.sql.DataSource;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.jdbc.core.CallableStatementCallback;
import org.springframework.jdbc.core.JdbcTemplate;
import org.springframework.stereotype.Repository;

/**
 *
 * @author digis
 */
@Repository
public class EstadoDAOImplementation implements EstadoDAO {

    private JdbcTemplate jdbcTemplate;

    @Autowired
    public EstadoDAOImplementation(DataSource dataSource) {
        this.jdbcTemplate = new JdbcTemplate(dataSource);
    }
    
    @Override
    public Result GetByIdPais(int idPais) {
        Result result = new Result();
        try{
            List<Estado> estados = jdbcTemplate.execute("{Call EstadoGetByIdPais(?,?)}", (CallableStatementCallback<List<Estado>>) callableStatementCallback ->{ 
               callableStatementCallback.registerOutParameter("pDatosCursor", Types.REF_CURSOR);
               callableStatementCallback.setInt("pIdPais", idPais);
               callableStatementCallback.execute();
               ResultSet rs = (ResultSet)callableStatementCallback.getObject("pDatosCursor");
               List<Estado> estadoList = new ArrayList();
               EstadoRowMapper estadoRowMapper = new EstadoRowMapper();
               while(rs.next()){
                   Estado estado = estadoRowMapper.mapRow(rs,rs.getRow());
                   estadoList.add(estado);
               }
               return estadoList;
            });
            
            if(estados != null){
                result.correct = true;
                result.object = estados;
            } else{
                result.correct = false;
                result.errorMessage = "La tabla alumno esta vacia";
            }
            
        }catch(Exception ex){
            result.correct = false;
            result.errorMessage = ex.getLocalizedMessage();
            result.exception = ex; 
        }
        return result;
    }
    
}
